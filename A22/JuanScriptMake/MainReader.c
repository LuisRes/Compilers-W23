 ...
 * -------------------------------------------------------------
 */
juan_void bErrorPrint(juan_char* fmt, ...);
juan_void displayBuffer(BufferReader* ptr_Buffer);
juan_long getFileSize(juan_char* fname);
juan_int isNumber(const juan_char* ns);
juan_void startReader(juan_char*, juan_char*, juan_char, juan_int, juan_int);
...
juan_int mainReader(juan_int argc, juan_char** argv) {

	/* Create source input buffer */
	juan_char* program = argv[0];
	juan_char* input = argv[2];
	juan_char mode = MODE_FIXED;
	juan_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
juan_void startReader(juan_char* program, juan_char* input, juan_char mode, juan_int size, juan_int increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	juan_int loadSize = 0;		/* the size of the file loaded in the buffer */
	juan_char symbol;			/* symbol read from input file */
