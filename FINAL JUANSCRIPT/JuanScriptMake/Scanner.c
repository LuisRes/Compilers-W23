...
Token funcSL(juan_char lexeme[]) {
	...
	if (readerIsEmpty(stringLiteralTable)) {
		stringLiteralTable = readerCreate(len + 2, len, MODE_ADDIT);
	}
	...
	if (!readerAddChar(stringLiteralTable, NL)) {
		currentToken.code = RTE_T;
		strcpy(currentToken.attribute.errLexeme, "Run Time Error:");
		errorNumber = RTE_CODE;
		return currentToken;
...
juan_void printToken(Token t) {
	...
	case MNID_T: //TOKEN 1
		...
		break;
	case VAR_T: //TOKEN 2
		printf("VAR_T\t\t%s\t ", t.attribute.idLexeme);
		printf("\n");
		break;
	case STR_T: //TOKEN 3
		printf("STR_T\t%d\t", (juan_int)t.attribute.codeType);
		printf("%s", readerGetContent(stringLiteralTable, (juan_int)t.attribute.codeType));
		break;
	case LPR_T: //TOKEN 4
		printf("LPR_T\t\t(\t\n");
		break;
	...
	case ASS_T: //TOKEN 19
		...
		break;
	case COM_T: //TOKEN 20
		printf("COM_T\t\t/*Comment*/\t\n");
		break;
	case SEOF_T: //TOKEN 21
