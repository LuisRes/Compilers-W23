...
juan_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern juan_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern juan_int line; /* source code line number - defined in scanner.c */
...
extern juan_void startParser(juan_void);
extern juan_int startScanner(ReaderPointer sc_buf);

static juan_void printParserError(juan_char* fmt, ...);
static juan_void displayParser(ReaderPointer ptrBuffer);
static juan_long getParserFileSize(juan_char* fname);
static juan_void callGarbageCollector(juan_void);
...
juan_int mainParser(juan_int argc, juan_char** argv) {

	FILE* fi;       /* input file handle */
	juan_int loadsize = 0; /*the size of the file loaded in the buffer */
...
juan_void printParserError(juan_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(juan_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
juan_long getParserFileSize(juan_char* fname) {
	FILE* input;
	juan_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
juan_void displayParser(ReaderPointer ptrBuffer) {
...
juan_void callGarbageCollector(juan_void) {
